...
// Global options are extension getters added to the built-in Drun DSL.
// These can be defined in another file as is the case here or inlined anywhere
// in your makefile. Open up this file for more details about Global Options...
import './Makefile.opts.dart';

// Utils are extension methods added to the built-in Drun DSL.
// These can be defined in another file as is the case here or inlined anywhere
// in your makefile. Open up this file for more details about extending the DSL.
...
Future myTaskThatCallsAnother() async {
  await foo.build();
}
...
Future _myTaskThatRunsOnce() => task((drun) =>
...
Future myTaskThatRunsAChildProc() =>
...
Future myTaskThatRunsIfNotFound() => task((drun) async {
      if (!await drun.exists(['./bin/**/foo'])) {
        print(
            'You should only see me if the file `./bin/baz/foo` does not exist');
...
Future myTaskThatRunsIfChanged() => task((drun) async {
      if (await drun.changed(['./Makefile.dart'])) {
        print('You should only see me if this file has changed '
...
Future myTaskThatCopies() =>
    task((drun) => drun.copy('!/example/.env', '!/example/.env.copy'));
...
Future myTaskThatMoves() =>
    task((drun) => drun.move('!/example/.env.copy', '!/example/.env.moved'));
